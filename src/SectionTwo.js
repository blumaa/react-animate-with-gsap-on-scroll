...
function SectionTwo() {
  ...
  const sectionTwoAnimation = useRef(null);

  useEffect(() => {
    sectionTwoAnimation.current = gsap.timeline().addLabel('start').fromTo(sectionTwo.current, 1, {opacity: 0}, {opacity: 1}, 'start').from(sectionTwo.current, 1, {
      y: 200,
      ease: Power3.easeOut,
    }, 'start');

    return () => {
      sectionTwoAnimation.current.kill();
    };
  }, []);

  useEffect(() => {
    if (showSectionTwo) {
      sectionTwoAnimation.current.play();
    } else {
      sectionTwoAnimation.current.reverse();
    }
    console.log("showSectionTwo", showSectionTwo);
  }, [showSectionTwo]);
  ...
  return (
    <InView
      ...
      onChange={(inView, entry) => {
        ...
        // console.log("Inview SectionTwo:", inView);
      }}
      triggerOnce={false}
    >
